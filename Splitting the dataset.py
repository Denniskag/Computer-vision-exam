...
# Define paths
data_dir = 'training data'  # Local data directory in Colab
categories = ['motorcycles', 'saloon_cars']
train_dir = 'data_A/train'
test_dir = 'data_A/test'

# Create train and test directories in Google Drive if they don't exist
...
for category in categories:
    ...
    # Create category folders in train and test directories in Google Drive
    train_category_path = os.path.join(train_dir, category)
    test_category_path = os.path.join(test_dir, category)
    os.makedirs(train_category_path, exist_ok=True)
    os.makedirs(test_category_path, exist_ok=True)

    # Move images to train directory in Google Drive
    for img in train_images: 
        shutil.copy(img, os.path.join(train_category_path, os.path.basename(img)))

    # Move images to test directory in Google Drive
